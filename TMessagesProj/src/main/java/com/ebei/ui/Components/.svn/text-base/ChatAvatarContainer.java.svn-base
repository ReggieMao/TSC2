...
import com.ebei.ui.MediaActivity;
import com.ebei.ui.ProfileActivity;
...
//        subtitleTextView.setTextColor(Theme.getColor(Theme.key_actionBarDefaultSubtitle));
        subtitleTextView.setTextColor(context.getResources().getColor(R.color.textGray1));
                    ...
                    if (info != null && info.participants_count != 0) {
                        if (chat.megagroup && info.participants_count <= 200) {
                            if (onlineCount > 1 && info.participants_count != 0) {
                                UserPreference.putString(UserPreference.IS_GROUP, "yes");
