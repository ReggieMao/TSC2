...
import com.ebei.tsc.LocationController;
...
import com.ebei.ui.LocationActivity;
import com.ebei.ui.VoIPActivity;
        ...
        if (messageObject == null || messageObject.getId() == 0/* || !messageObject.isMusic()*/) {
            ...
            if (lastMessageObject != messageObject || prevStyle != 0) {
                ...
                if (lastMessageObject.isVoice() || lastMessageObject.isRoundVideo()) {
                    ...
                    String time = messageObject.getMusicTitle();
                    if (time.length() == 12) {
                        ...
                        time = (month + time.substring(4, time.length())).replace(" ", "日");
                    }
                    stringBuilder = new SpannableStringBuilder(String.format("%s %s", messageObject.getMusicAuthor(), time));
