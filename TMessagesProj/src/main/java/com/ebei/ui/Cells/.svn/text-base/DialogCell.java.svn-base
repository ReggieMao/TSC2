...
import com.ebei.ui.Components.GroupCreateCheckBox;
...
//        lastMessageString = message != null ? message.messageText : null;
        lastMessageString = message != null ? message.messageTransfer : null;
...
//                    stringBuilder = SpannableStringBuilder.valueOf(String.format(messageFormat, name, message.messageText));
                    stringBuilder = SpannableStringBuilder.valueOf(String.format(messageFormat, name, message.messageTransfer));
                    ...
                    if (Util.isRedPac(mess)) {
                        mess = LocaleController.getString("RedEnvelope", R.string.RedEnvelope);
                    }
                    if (Util.isTransfer(mess)) {
                        mess = LocaleController.getString("ChatTransfer", R.string.ChatTransfer);
                    }
                    if (Util.isRedReceive(mess)) {
                        mess = LocaleController.getString("ActionYouReceiveRed", R.string.ActionYouReceiveRed).replace("un1", mess.substring(mess.indexOf("$") + 1, mess.length() - 5));
                    }
                    if (mess.length() > 150) {
                        mess = mess.substring(0, 150);
                    }
                    stringBuilder = SpannableStringBuilder.valueOf(String.format(messageFormat, name, mess.replace('\n', ' ')));
                    if (Util.isRedPac(customDialog.message) || Util.isTransfer(customDialog.message) || Util.isRedReceive(customDialog.message)) {
                        stringBuilder.setSpan(new ForegroundColorSpan(Theme.getColor(Theme.key_chats_attachMessage)), name.length() + 2, stringBuilder.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
                    }
                }
...
//                                messageString = message.messageText;
                                messageString = message.messageTransfer;
                            }
                            currentMessagePaint = Theme.dialogs_messagePrintingPaint;
                        } else {
                            if (chat != null && chat.id > 0 && fromChat == null) {
                                ...
                                if (message.caption != null) {
                                    ...
                                    stringBuilder = SpannableStringBuilder.valueOf(String.format(messageFormat, name, mess.replace('\n', ' ')));
                                    if (Util.isRedPac(message.caption.toString()) || Util.isTransfer(message.caption.toString()) || Util.isRedReceive(message.caption.toString())) {
                                        stringBuilder.setSpan(new ForegroundColorSpan(Theme.getColor(Theme.key_chats_attachMessage)), name.length() + 2, stringBuilder.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
                                    }
...
//                                        stringBuilder = SpannableStringBuilder.valueOf(String.format(messageFormat, name, message.messageText));
                                    ...
                                    stringBuilder = SpannableStringBuilder.valueOf(String.format(messageFormat, name, mess.replace('\n', ' ')));
                                    if (Util.isRedPac(message.messageOwner.message) || Util.isTransfer(message.messageOwner.message) || Util.isRedReceive(message.messageOwner.message)) {
                                        stringBuilder.setSpan(new ForegroundColorSpan(Theme.getColor(Theme.key_chats_attachMessage)), name.length() + 2, stringBuilder.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
                                    }
                                } else {
...
//                                        messageString = message.messageText;
                                    ...
                                    if (message.messageOwner.media != null && !message.isMediaEmpty()) {
                                        currentMessagePaint = Theme.dialogs_messagePrintingPaint;
                                    } else if (messageString.equals(LocaleController.getString("RedEnvelope", R.string.RedEnvelope))) {
                                        currentMessagePaint = Theme.dialogs_messagePrintingPaint;
                                    } else if (messageString.equals(LocaleController.getString("ChatTransfer", R.string.ChatTransfer))) {
                                        currentMessagePaint = Theme.dialogs_messagePrintingPaint;
                                    } else if (messageString.toString().endsWith("领取了红包")) {
                                        currentMessagePaint = Theme.dialogs_messagePrintingPaint;
                                    }
                                }
                            }
...
//                    if (message != null && message.messageText != lastMessageString) {
                    if (message != null && message.messageTransfer != lastMessageString) {
                        continueUpdate = true;
                    }
                }
