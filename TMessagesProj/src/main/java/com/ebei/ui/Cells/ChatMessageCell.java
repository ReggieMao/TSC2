...
import com.ebei.tsc.WebFile;
import com.ebei.tsc.browser.Browser;
import com.ebei.ui.ActionBar.Theme;
import com.ebei.ui.Components.AvatarDrawable;
import com.ebei.ui.Components.LinkPath;
import com.ebei.ui.Components.RadialProgress;
import com.ebei.ui.Components.RoundVideoPlayingDrawable;
import com.ebei.ui.Components.SeekBar;
import com.ebei.ui.Components.SeekBarWaveform;
import com.ebei.ui.Components.StaticLayoutEx;
...
import com.ebei.ui.SecretMediaViewer;
...
public class ChatMessageCell extends BaseCell implements SeekBar.SeekBarDelegate, ImageReceiver.ImageReceiverDelegate, DownloadController.FileDownloadProgressListener {
    ...
    public void setMessageObject(MessageObject messageObject, MessageObject.GroupedMessages groupedMessages, boolean bottomNear, boolean topNear) {
        ...
        if (messageChanged || dataChanged || groupChanged || isPhotoDataChanged(messageObject) || pinnedBottom != bottomNear || pinnedTop != topNear) {
            ...
            if (messageObject.type == 0) { // 消息类型为文本
                if (!Util.isRedPac(messageObject.messageText.toString()) && !Util.isTransfer(messageObject.messageText.toString())
//                        && !Util.isRedReceive(messageObject.messageText.toString())
                        ) { // 普通文本消息
                ...
                } else if (Util.isRedPac(messageObject.messageText.toString())) { // 红包文本消息
                    drawName = false;
                    drawForwardedName = false;
                    drawPhotoImage = false;
                    if (AndroidUtilities.isTablet()) {
                    ...
                    String time = "领取红包\n" + LocaleController.getInstance().formatterDay.format((long) (messageObject.messageOwner.date) * 1000);
                    String text = messageObject.messageText.toString().split("[$]")[2].substring(0, messageObject.messageText.toString().split("[$]")[2].indexOf("}"));
                    titleLayout = new StaticLayout(text, Theme.chat_audioTitlePaint, 500, Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false);
                    ...
                    totalHeight = AndroidUtilities.dp(90) + namesOffset;
                    if (drawPinnedTop) {
                        namesOffset -= AndroidUtilities.dp(1);
                    }
                } else if (Util.isTransfer(messageObject.messageText.toString())) { // 转账文本消息
                    drawName = false;
                    drawForwardedName = false;
                    drawPhotoImage = false;
                    if (AndroidUtilities.isTablet()) {
                    ...
                    String time = "转账\n" + LocaleController.getInstance().formatterDay.format((long) (messageObject.messageOwner.date) * 1000);
                    String text = messageObject.messageText.toString().split("[$]")[1] + " " + messageObject.messageText.toString().split("[$]")[2].substring(0, messageObject.messageText.toString().split("[$]")[2].indexOf("}"));
                    titleLayout = new StaticLayout(text, Theme.chat_audioTitlePaint, 500, Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false);
                    docTitleLayout = new StaticLayout(TextUtils.ellipsize(time, Theme.chat_contactPhonePaint, maxWidth, TextUtils.TruncateAt.END), Theme.chat_contactPhonePaint, maxWidth + AndroidUtilities.dp(2), Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false);
    ...
    private void drawContent(Canvas canvas) {
            ...
            if (currentMessageObject.textLayoutBlocks != null && !currentMessageObject.textLayoutBlocks.isEmpty()) {
                ...
                if (firstVisibleBlockNum >= 0) {
                    for (int a = firstVisibleBlockNum; a <= lastVisibleBlockNum; a++) {
                        ...
                        try {
                            if (!Util.isRedPac(currentMessageObject.messageText.toString()) && !Util.isTransfer(currentMessageObject.messageText.toString()) && !Util.isRedReceive(currentMessageObject.messageText.toString()))
        ...
        if (currentMessageObject.type == 1 || documentAttachType == DOCUMENT_ATTACH_TYPE_VIDEO) {
            ...
            if (currentMessageObject.type == 4) {
            ...
            } else if (currentMessageObject.type == 0 && Util.isRedPac(currentMessageObject.messageText.toString())) {
                Theme.chat_audioTitlePaint.setColor(Theme.getColor(Theme.key_chat_redGolden));
                Theme.chat_contactPhonePaint.setColor(Theme.getColor(Theme.key_chat_redGolden));
                forceNotDrawTime = true;
                int x;
                if (currentMessageObject.isOutOwner()) {
                ...
                if (docTitleLayout != null) {
                    canvas.save();
                    canvas.translate(x, AndroidUtilities.dp(50) + namesOffset);
                    docTitleLayout.draw(canvas);
                    canvas.restore();
                }
                Drawable redPag = Theme.chat_msgRedPagDrawable;
                int my;
                if (currentMessageObject.isOutOwner()) {
                    my = layoutWidth - backgroundWidth + AndroidUtilities.dp(16);
                } else {
                    if (isChat && currentMessageObject.needDrawAvatar()) {
                        my = AndroidUtilities.dp(61);
                    } else {
                        my = AndroidUtilities.dp(25);
                    }
                }
                setDrawableBounds(redPag, my + AndroidUtilities.dp(180), otherY = AndroidUtilities.dp(15));
                redPag.draw(canvas);
            } else if (currentMessageObject.type == 0 && Util.isTransfer(currentMessageObject.messageText.toString())) {
                Theme.chat_audioTitlePaint.setColor(Theme.getColor(Theme.key_chat_serviceText));
                Theme.chat_contactPhonePaint.setColor(Theme.getColor(Theme.key_chat_serviceText));
                forceNotDrawTime = true;
                int x;
                if (currentMessageObject.isOutOwner()) {
                    x = layoutWidth - backgroundWidth + AndroidUtilities.dp(16);
                ...
                Drawable transfer = Theme.chat_msgTransferDrawable;
                int my;
                if (currentMessageObject.isOutOwner()) {
                    my = layoutWidth - backgroundWidth + AndroidUtilities.dp(16);
                } else {
                ...
                setDrawableBounds(transfer, my + AndroidUtilities.dp(180), otherY = AndroidUtilities.dp(15));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (currentMessageObject.isOutOwner()) {
            if (!mediaBackground && !drawPinnedBottom) {
                if (currentMessageObject.type == 0) {
                    if (Util.isRedPac(currentMessageObject.messageText.toString())) {
                        currentBackgroundDrawable = Theme.chat_msgOutDrawableRedPag;
                        currentBackgroundSelectedDrawable = Theme.chat_msgOutSelectedDrawableRedPag;
                    } else if (Util.isTransfer(currentMessageObject.messageText.toString())) {
                        currentBackgroundDrawable = Theme.chat_msgOutDrawableTransfer;
                        currentBackgroundSelectedDrawable = Theme.chat_msgOutSelectedDrawableTransfer;
                    } else {
                ...
                if (currentMessageObject.type == 0) {
                    if (Util.isRedPac(currentMessageObject.messageText.toString())) {
                        currentBackgroundDrawable = Theme.chat_msgOutMediaDrawableRedPag;
                        currentBackgroundSelectedDrawable = Theme.chat_msgOutMediaSelectedDrawableRedPag;
                    } else if (Util.isTransfer(currentMessageObject.messageText.toString())) {
                        currentBackgroundDrawable = Theme.chat_msgOutMediaDrawableTransfer;
                        currentBackgroundSelectedDrawable = Theme.chat_msgOutMediaSelectedDrawableTransfer;
                    } else {
            ...
            if (!mediaBackground && !drawPinnedBottom) {
                if (currentMessageObject.type == 0) {
                    if (Util.isRedPac(currentMessageObject.messageText.toString())) {
                        currentBackgroundDrawable = Theme.chat_msgInDrawableRedPag;
                        currentBackgroundSelectedDrawable = Theme.chat_msgInSelectedDrawableRedPag;
                    } else if (Util.isTransfer(currentMessageObject.messageText.toString())) {
                        currentBackgroundDrawable = Theme.chat_msgInDrawableTransfer;
                        currentBackgroundSelectedDrawable = Theme.chat_msgInSelectedDrawableTransfer;
                    } else {
                ...
                if (currentMessageObject.type == 0) {
                    if (Util.isRedPac(currentMessageObject.messageText.toString())) {
                        currentBackgroundDrawable = Theme.chat_msgInMediaDrawableRedPag;
                        currentBackgroundSelectedDrawable = Theme.chat_msgInMediaSelectedDrawableRedPag;
                    } else if (Util.isTransfer(currentMessageObject.messageText.toString())) {
                        currentBackgroundDrawable = Theme.chat_msgInMediaDrawableTransfer;
                        currentBackgroundSelectedDrawable = Theme.chat_msgInMediaSelectedDrawableTransfer;
                    } else {
