...
import com.ebei.tgnet.SerializedData;
...
import com.ebei.ui.Components.URLSpanBrowser;
import com.ebei.ui.Components.URLSpanMono;
import com.ebei.ui.Components.URLSpanNoUnderline;
import com.ebei.ui.Components.URLSpanNoUnderlineBold;
...
public class MessageObject {
    ...
    public CharSequence messageTransfer;
    ...
    public MessageObject(int accountNum, TLRPC.Message message, String formattedMessage, String name, String userName, boolean localMessage, boolean isChannel) {
        ...
        messageTransfer = formattedMessage;
    ...
    public MessageObject(int accountNum, TLRPC.Message message, AbstractMap<Integer, TLRPC.User> users, AbstractMap<Integer, TLRPC.Chat> chats, SparseArray<TLRPC.User> sUsers, SparseArray<TLRPC.Chat> sChats, boolean generateLayout, long eid) {
        ...
        if (message.message != null) {
            if (message.message.startsWith(Constants.RED_RECEIVE_CODE1) && message.message.endsWith(Constants.RED_RECEIVE_CODE2)) {
                messageText = LocaleController.getString("ActionYouReceiveRed", R.string.ActionYouReceiveRed).replace("un1", message.message.substring(message.message.indexOf("$") + 1, message.message .length() - 5));
                message = new TLRPC.TL_messageService();
                message.media = null;
                message.action = new TLRPC.TL_messageActionChatEditTitle();
            }
        }
        ...
        if (Util.isRedPac(messageText.toString()))
            messageTransfer = LocaleController.getString("RedEnvelope", R.string.RedEnvelope);
        else if (Util.isTransfer(messageText.toString()))
            messageTransfer = LocaleController.getString("ChatTransfer", R.string.ChatTransfer);
        else if (messageText.equals("joined Telegram"))
            messageTransfer = "加入了TSC";
        else
            messageTransfer = messageText;
