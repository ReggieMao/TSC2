...
/* package */ class DefaultDrmSession<T extends ExoMediaCrypto> implements DrmSession<T> {
  ...
  private void postKeyRequest(int type, boolean allowRetry) {
    ...
    try {
      ExoMediaDrm.KeyRequest mediaDrmKeyRequest =
      ...
      try {
        switch (msg.what) {
          case MSG_PROVISION:
            ...
            break;
          case MSG_KEYS:
            Pair<ExoMediaDrm.KeyRequest, String> keyRequest = (Pair<ExoMediaDrm.KeyRequest, String>) request;
            ExoMediaDrm.KeyRequest mediaDrmKeyRequest = keyRequest.first;
